...
def flatten_node(hash)
  ...
  fields.each do |key, value|
    ...
    when String, Symbol
      $lines << "S #{id} #{key} #{escape_str(value.to_s)}"
    when true
      $lines << "I #{id} #{key} 1"
    when false
      $lines << "I #{id} #{key} 0"
...
dest_file = ARGV[1]
unless source_file
  $stderr.puts "Usage: ruby spinel_parse.rb input.rb [output.ast]"
  exit 1
end

source = File.read(source_file, encoding: "utf-8")
unless File.basename(source_file) == "array.rb"
...
output_lines = []
output_lines << "ROOT #{root_id}"
$lines.each { |l| output_lines << l }

if dest_file
  File.write(dest_file, output_lines.join("\n") + "\n", encoding: "utf-8")
  $stderr.puts "Parsed AST to: #{dest_file}"
else
